...
public class DemosiCommand {
    ...
    public DemosiCommand(PartaiCore plugin) {
    ...
    public boolean execute(CommandSender sender, String[] args) {
        ...
        if (!playerMember.getRole().equalsIgnoreCase("ketua")) {
            MessageUtils.sendError(sender, "Hanya ketua partai yang bisa mendemosikan anggota!");
            return true;
        }
        ...
        // Tidak bisa demosi ke ketua
        if (newRole.equalsIgnoreCase("ketua")) {
            MessageUtils.sendError(sender, "Tidak bisa mendemosikan ke jabatan ketua!");
            return true;
        }

        // Validasi: target tidak demosi diri sendiri
        if (targetUuid.equals(playerUuid)) {
            MessageUtils.sendError(sender, "Kamu tidak bisa mendemosikan diri sendiri!");
            return true;
        }
        ...
        // Validasi: harus demosi (level turun)
        if (newLevel >= currentLevel) {
            MessageUtils.sendError(sender, "Demosi harus menurunkan jabatan! Gunakan /partai promosi untuk menaikkan.");
            return true;
        }
        ...
        // Notifikasi
        MessageUtils.sendSuccess(sender, targetName + " berhasil didemosikan dari " + currentRole + " menjadi " + newRole);
